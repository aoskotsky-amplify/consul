...
func CheckTypeToStructs(s CheckType) (structs.CheckType, error) {
	...
	t.Body = s.Body
	t.TCP = s.TCP
	interval, err := types.DurationFromProto(&s.Interval)
	if err != nil {
		return t, err
	}
	t.Interval = interval
	...
	timeout, err := types.DurationFromProto(&s.Timeout)
	if err != nil {
		return t, err
	}
	t.Timeout = timeout
	ttl, err := types.DurationFromProto(&s.TTL)
	if err != nil {
		return t, err
	}
	t.TTL = ttl
	...
	deregisterCriticalServiceAfter, err := types.DurationFromProto(&s.DeregisterCriticalServiceAfter)
	if err != nil {
		return t, err
	}
...
func NewCheckTypeFromStructs(t structs.CheckType) CheckType {
	...
	s.Body = t.Body
	s.TCP = t.TCP
	s.Interval = *types.DurationProto(t.Interval)
	...
	s.Timeout = *types.DurationProto(t.Timeout)
	s.TTL = *types.DurationProto(t.TTL)
	...
	s.DeregisterCriticalServiceAfter = *types.DurationProto(t.DeregisterCriticalServiceAfter)
